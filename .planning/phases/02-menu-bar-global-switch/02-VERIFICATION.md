...
| 3 | The menu lists checked recommended full-support editors first, leaves unchecked eligible editors in `More`, and preserves the curated default order with `Kiro` immediately after `Cursor`, `Qoder` after `Zed`, and `TextEdit` included in the default enabled set. | ✓ VERIFIED | `App/Features/MenuBar/MenuBarViewModel.swift`, `App/Application/Settings/RecommendedMenuAppsStore.swift`, and `App/Support/KnownEditors.swift` now build first-level rows strictly from checked available recommendations without backfill or current-editor injection; `WorkspaceAppDiscovery.swift`, `BundleDocumentTypeReader.swift`, and `EditorRankingPolicy.swift` still preserve the curated/global ordering guarantees for the remaining eligible candidates; `Tests/DefaultEditorSwitcherTests/MenuBarViewModelTests.swift`, `Tests/DefaultEditorSwitcherTests/RecommendedMenuAppsStoreTests.swift`, and `Tests/DefaultEditorSwitcherTests/EditorRankingPolicyTests.swift` assert the new behavior. |
...
| `Tests/DefaultEditorSwitcherTests/MenuBarViewModelTests.swift` | Menu load/apply/window exposure coverage | ✓ EXISTS + SUBSTANTIVE | Covers loading, checked-first primary ordering, no-backfill first-level behavior, current-editor overflow behavior, apply reload, and rules-window action exposure. |
